...
import { Box, Divider, Grid, Button, Typography, FormGroup, FormControlLabel, Switch } from "@mui/material"
...
const MaterialUISwitch = styled(Switch)(({ theme }) => ({
  ...
  '& .MuiSwitch-switchBase': {
    ...
    '&.Mui-checked': {
      ...
      '& + .MuiSwitch-track': {
        opacity: 1,
        backgroundColor: theme.palette.mode === 'dark' ? '#666171' : '#aab4be',
      },
    },
  },
  '& .MuiSwitch-thumb': {
    ...
    <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'} mt={5}>
      <Box display={'flex'} alignItems={'center'}>
        ...
        <Box display={'flex'} alignItems={'center'} ml={0}>
          ...
          <Typography sx={{
            color: palette.text.primary, marginRight: '10px', cursor: 'pointer'
          }}>EN</Typography>
        </Box>
      </Box>
      <Box display={'flex'} alignItems={'center'}>
        <Box display='flex' alignItems='center' mr={4} sx={{cursor: 'pointer'}}>
          <Box sx={{
            width: '24px',
            '&:hover': {
              width: '28px',
        ...
        Object.keys(navItems).map((key, i) => {
          ...
          return (
            <Grid key={i} item md={3} sm={6} xs={12}>
              { navItem.length > 0 && navItem.map((item, i)=>(
                i === 0 ?
                  <Typography my={2} key={i} sx={{
                    color: i === 0 ? '#9a6aff' : 'white',
                    fontSize: 14,
                    fontWeight: 600,
                  }}>{item.text}</Typography>
                :
                <Typography my={2} key={i}>
                  <Link to={item.to} style={{ color: 'white', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
                    {item.text}
                  </Link>
                </Typography>
              ))}
